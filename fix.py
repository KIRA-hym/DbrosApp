...
with codecs.open(r'C:\dbros_app\lib\screens\settings_page.dart', 'r', encoding='utf-8') as f:
    code = f.read()

# 1. Add _showFeeInsuranceDialog inside _SettingsPageState
if '_showFeeInsuranceDialog' not in code:
    if "import '../services/overlay_manager.dart';" not in code:
        code = code.replace("import '../services/settings_service.dart';", "import '../services/settings_service.dart';\nimport '../services/overlay_manager.dart';")
...
"""
    idx = code.find('  Widget _buildSettingsScrollBody')
    code = code[:idx] + dialog_code + "\\n" + code[idx:]

# 2. Replace _buildSettingsScrollBody's contents
body_match = re.search(r'Container\(\s*key: _keyFeeInsurance.*?_buildSettingsGroup\(\s*"보험료 설정".*?\),', code, re.DOTALL)
if body_match:
    replacement = '''      _buildListManageButton(
        title: '수수료 및 보험료 설정',
        icon: Icons.monetization_on_outlined,
        onTap: _showFeeInsuranceDialog,
      ),'''
    code = code.replace(body_match.group(0), replacement)

# 3. Replace the 3 old settings with _buildAppConvenienceSettings inside _buildSettingsScrollBody
old_convenience = re.search(r'if \(!kIsWeb && Platform\.isAndroid\)\s*Container\(\s*key: _keyStatusBarQuick,\s*child: _buildStatusBarQuickSettings\(\),\s*\),\s*_buildFloatingButtonSettings\(\),\s*if \(!kIsWeb && Platform\.isAndroid\)\s*Container\(\s*key: _keyScreenshotAuto,\s*child: _buildScreenshotAutoRegisterSettings\(\),\s*\),', code, re.DOTALL)
if old_convenience:
    code = code.replace(old_convenience.group(0), '''      if (!kIsWeb && Platform.isAndroid)
        Container(
...
old_methods = [
    r'  Widget _buildStatusBarQuickSettings\(\) \{.*?(?=  Widget _buildFloatingButtonSettings|  Widget _buildProModeTestToggle|  Widget _buildScreenshotAuto)',
    r'  Widget _buildFloatingButtonSettings\(\) \{.*?(?=  Widget _buildScreenshotAuto|  Widget _buildProModeTestToggle|  Widget _buildSettingsScrollBody)',
    r'  Widget _buildScreenshotAutoRegisterSettings\(\) \{.*?(?=  Widget _buildOcrParseLogSettings|  Widget _buildBackupRestore)'
]
for pattern in old_methods:
    match = re.search(pattern, code, re.DOTALL)
    if match:
        code = code.replace(match.group(0), '')

with codecs.open(r'C:\dbros_app\lib\screens\settings_page.dart', 'w', encoding='utf-8') as f:
    f.write(code)
