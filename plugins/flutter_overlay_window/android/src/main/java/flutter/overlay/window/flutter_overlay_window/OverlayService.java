...
public class OverlayService extends Service implements View.OnTouchListener {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        flutterChannel.setMethodCallHandler((call, result) -> {
            ...
            } else if (call.method.equals("openPopupActivity")) {
                try {
                    Intent popupIntent = new Intent(getApplicationContext(), Class.forName("com.dbros.drive.PopupActivity"));
                    popupIntent.setAction(Intent.ACTION_RUN);
    ...
    public void onCreate() {
        ...
        int customIconId = getDrawableResourceId("drawable", "app_notification_icon");
        if (customIconId == 0) customIconId = getDrawableResourceId("mipmap", "ic_launcher");
        if (customIconId == 0) customIconId = R.drawable.notification_icon;
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            // 오버레이 백그라운드 서비스 알림이 런처 앱 아이콘에 빨간 뱃지(점)를 띄우지 않도록 방지
