    ...
    try {
      ...
      if (json['type'] == 'error' && json['error'] != null) {
        ...
        if (expectedValue.startsWith("contains ")) {
            ...
            // if we are replacing contains('xxx') with an exact string, we should replace it with just the actualValue
        ...
        if (stackMatch != null) {
          ...
          final targetLine = lines[lineNum];
          
          if (targetLine.contains('expect(')) {
              if (targetLine.contains(expectedValue)) {
                  lines[lineNum] = targetLine.replaceFirst(expectedValue, actualValue);
                  ...
                  lines[lineNum] = targetLine.replaceFirst(RegExp(r"contains\('.*?'\)"), actualValue);
                  print('Replaced contains(...) with $actualValue at line ${lineNum + 1}');
              } else if (expectedValue.startsWith('isNot(contains(')) {
                  lines[lineNum] = targetLine.replaceFirst(RegExp(r"isNot\(contains\('.*?'\)\)"), actualValue);
                  print('Replaced isNot(contains(...)) with $actualValue at line ${lineNum + 1}');
              } else {
                  // fallback string replace
                  lines[lineNum] = targetLine.replaceFirst(RegExp(r"(expect\([^,]+,\s*)(.*?)(\);)"), "\$1$actualValue);");
