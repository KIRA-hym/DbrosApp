...
### 📝 작업 일지 및 백업(복원 지점) 관리 (필수)
- **당일 작업일지 생성 및 누적 기록**: 작업 수행 시 반드시 `work` 폴더 내에 오늘 날짜의 작업일지(예: `YYYYMMDD.md`)를 만들고, 작업 계획에 맞는 수행 결과와 누적 작업 내역을 계속해서 기록할 것.
- **복원 지점(백업) 명시 및 롤백 지원**: 작업 중 안전하게 돌아갈 수 있는 백업 지점(Git 커밋 해시 또는 브랜치 상태)을 작업일지에 반드시 명시해 둘 것. 사용자가 롤백을 요청하면 작업일지를 참고하여 원하는 복원 지점으로 즉시 롤백을 진행할 수 있어야 함.
- 커밋·푸시 완료 후 **변경 파일, 변경 내용, 이유**를 한글로 정리하여 보고할 것.

### 🛡️ 파싱 로직 절대 보호 규칙
- **현재 시점 완전성 유지**: OCR 및 텍스트 파싱 로직(`lib/utils/logi_colmanner_ocr.dart` 등)은 현재가 **가장 완벽한 시점**임.
- 추후 파싱 로직을 보완해야 할 일이 생기더라도 기존 로직 흐름을 절대 깨뜨리지 않도록 각별히 유념하고 보완 작업을 수행할 것.
